...
struct BoseChaudhuriHocquenghemGenerator {
    static void matrix(int8_t* output, bool systematic, std::initializer_list<int> factors) {
        static_assert(N <= 255);
        // Multiply the supplied binary generator-polynomial factors over GF(2).
        ...
        // Rows are shifted copies of the high-to-low coefficient vector.
        for (int row = 0; row < K; ++row)
            for (int offset = 0; offset <= N - K; ++offset)
                output[row * N + row + offset] =
                    static_cast<int8_t>(polynomial[static_cast<std::size_t>(offset)]);
        if (!systematic) return;
        // The first K columns are nonsingular for this orientation. Row-reduce
        // them to produce the deterministic systematic matrix used by the
...
class OrderedStatisticsDecoder {
    ...
    static int metric(const int8_t* candidate, const int8_t* soft) {
        ...
        for (int index = 0; index < N; ++index)
            value += (1 - 2 * static_cast<int>(candidate[index] & 1)) *
                static_cast<int>(soft[index]);
        return value;
    }

public:
    bool operator()(uint8_t* decoded, const int8_t* input_soft,
                    const int8_t* input_generator) {
        ...
        for (int column = 0; column < N; ++column) {
            ...
            for (int row = 0; row < K; ++row)
                matrix[static_cast<std::size_t>(row * W + column)] =
                    input_generator[row * N + original] & 1;
        }

        // Reliability-ordered Gaussian elimination with column exchanges when
        // a sorted coordinate is dependent on the already chosen basis.
        for (int pivot = 0; pivot < K; ++pivot) {
            ...
            while (true) {
                pivot_row = pivot;
                while (pivot_row < K &&
                       !matrix[static_cast<std::size_t>(pivot_row * W + pivot_column)])
                    ++pivot_row;
            ...
            if (pivot_column != pivot) {
                for (int row = 0; row < K; ++row)
                    std::swap(matrix[static_cast<std::size_t>(row * W + pivot)],
                              matrix[static_cast<std::size_t>(row * W + pivot_column)]);
                std::swap(soft[static_cast<std::size_t>(pivot)],
                          soft[static_cast<std::size_t>(pivot_column)]);
                std::swap(permutation[static_cast<std::size_t>(pivot)],
                          permutation[static_cast<std::size_t>(pivot_column)]);
            }
            if (pivot_row != pivot)
                for (int column = 0; column < W; ++column)
                    std::swap(matrix[static_cast<std::size_t>(pivot * W + column)],
                              matrix[static_cast<std::size_t>(pivot_row * W + column)]);
            for (int row = 0; row < K; ++row) {
                if (row == pivot ||
                    !matrix[static_cast<std::size_t>(row * W + pivot)]) continue;
                for (int column = 0; column < W; ++column)
                    matrix[static_cast<std::size_t>(row * W + column)] ^=
                        matrix[static_cast<std::size_t>(pivot * W + column)];
            }
        }
        ...
        overridden = CODE_OSD_SEARCH_OVERRIDE(
            matrix.data(), base.data(), candidate.data(), soft.data(),
            permutation.data(), N, K, W, O, best, next
        );
#endif
        if (!overridden) {
            int absolute = 0;
            for (int index = 0; index < N; ++index)
                absolute += std::abs(static_cast<int>(soft[static_cast<std::size_t>(index)]));
            ...
            std::array<int, O> selected{};
            auto evaluate = [&](int weight) {
                int distance = 0;
                for (int index = 0; index < N; ++index)
                    if ((trial[static_cast<std::size_t>(index)] != 0) !=
                        (soft[static_cast<std::size_t>(index)] < 0))
            ...
            auto recurse = [&](auto&& self, int start, int depth, int target) -> void {
                if (depth == target) { evaluate(target); return; }
                for (int rank = start; rank <= K - (target - depth); ++rank) {
                    selected[static_cast<std::size_t>(depth)] = rank;
                    for (int column = 0; column < W; ++column)
                        trial[static_cast<std::size_t>(column)] ^=
                            matrix[static_cast<std::size_t>(rank * W + column)];
            ...
            for (int weight = 1; weight <= O; ++weight)
                recurse(recurse, 0, 0, weight);
            best = absolute - 2 * best_distance;
            next = next_distance == std::numeric_limits<int>::max()
                ? -1 : absolute - 2 * next_distance;
        }
        ...
        for (int column = 0; column < N; ++column)
            set_be_bit(decoded, permutation[static_cast<std::size_t>(column)],
                       candidate[static_cast<std::size_t>(column)] != 0);
        return next != best;
    }
};
