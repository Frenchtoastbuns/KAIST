...
module cap_r2_normal_core #(
    ...
    generate
        for (mg=0; mg<GROUPS; mg=mg+1) begin: g_r2_mem
            cap_r2_tdp_ram #(.DEPTH(R2_DEPTH),.ADDR_W(R2_AW),.DATA_W(10)) mem (
                .clk(clk),
    ...
    reg [1:0] q_count;
    reg [1:0] q_issue_index;
    reg [1:0] q_weight_rows [0:1];
    reg [METRIC_W-1:0] q_info_rows [0:1];
    reg q_eval_valid;
    reg q_eval_last;
    ...
    cap_r2_rank_pair_ram #(.DEPTH(TASK_DEPTH),.ADDR_W(TASK_AW)) task_mem (
    ...
    always @* begin
        ...
        end else if (q_active && q_issue_index < q_count) begin
            for (cg=0; cg<GROUPS; cg=cg+1) begin
                r2_a_en[cg]=1;
                r2_a_address[cg]=r2_addr(q_suffix,q_weight_rows[q_issue_index],
                    packed_group_state(q_states,cg));
            end
        end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (q_eval_valid) begin
                ...
                for (sumg=0;sumg<GROUPS;sumg=sumg+1)
                    query_total=query_total+r2_a_q[sumg];
                if (query_total < q_minimum_seen)
                    q_minimum_seen<=query_total[METRIC_W-1:0];
                query_pass_now=query_total<=q_threshold;
                bound_row_cycles<=bound_row_cycles+1;
                if (query_pass_now || q_eval_last) begin
                    q_active<=0; q_eval_valid<=0; q_result_valid<=1;
