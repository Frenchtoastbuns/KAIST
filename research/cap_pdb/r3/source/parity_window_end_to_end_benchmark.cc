...
enum class Mode {
	...
	reconstructed_fused_threshold_dual_exact,
	reconstructed_fused_oracle_dual_exact,
	reconstructed_partial_h4_dual_exact,
	reconstructed_partial_h6_dual_exact,
	reconstructed_partial_h8_dual_exact,
	r3_specialized_dual_exact,
	r3_fused_specialized_dual_exact,
	r3_fused_w1_oracle_exact,
	r3_fused_w1w2_oracle_exact,
...
const char *mode_name(Mode mode)
{
	...
	case Mode::reconstructed_fused_threshold_dual_exact:
		return "reconstructed_fused_threshold_dual_exact";
	case Mode::reconstructed_fused_oracle_dual_exact:
		return "reconstructed_fused_oracle_dual_exact";
	case Mode::reconstructed_partial_h4_dual_exact:
		return "reconstructed_partial_h4_dual_exact";
	case Mode::reconstructed_partial_h6_dual_exact:
		return "reconstructed_partial_h6_dual_exact";
	case Mode::reconstructed_partial_h8_dual_exact:
		return "reconstructed_partial_h8_dual_exact";
	case Mode::r3_specialized_dual_exact:
		return "r3_specialized_dual_exact";
	case Mode::r3_fused_specialized_dual_exact:
		return "r3_fused_specialized_dual_exact";
	case Mode::r3_fused_w1_oracle_exact:
		return "r3_fused_w1_oracle_exact";
	case Mode::r3_fused_w1w2_oracle_exact:
		return "r3_fused_w1w2_oracle_exact";
...
struct OverrideStats {
	...
	uint64_t dual_updates_executed = 0;
	uint64_t dual_group_solves = 0;
	uint64_t compact_weight_certificates = 0;
	uint64_t dual_weight_certificates = 0;
	uint64_t witness_early_stops = 0;
	uint64_t exact_oracle_candidates = 0;
	uint64_t exact_oracle_metric_terms = 0;
	uint64_t exact_oracle_primitive_ops = 0;
	uint64_t dual_witness_candidates = 0;
	uint64_t dual_witness_metric_terms = 0;
	uint64_t dual_witness_primitive_ops = 0;
	uint64_t r3_structural_checks = 0;
	std::array<uint64_t, 5> exact_oracle_candidates_by_weight{};
	std::array<uint64_t, 5> exact_oracle_primitive_ops_by_weight{};
	std::array<uint64_t, 5> dual_operations_by_weight{};
	std::array<uint64_t, 5> dual_group_solves_by_weight{};
	std::array<uint64_t, 4> pair_parallel_cycles{};
...
public:
	...
	struct DualDecisionStats {
		uint64_t operations = 0;
		uint64_t updates = 0;
		uint64_t group_solves = 0;
		uint64_t compact_certificates = 0;
		uint64_t dual_certificates = 0;
		uint64_t witness_stops = 0;
		uint64_t oracle_candidates = 0;
		uint64_t oracle_metric_terms = 0;
		uint64_t oracle_primitive_ops = 0;
		uint64_t witness_candidates = 0;
		uint64_t witness_metric_terms = 0;
		uint64_t witness_primitive_ops = 0;
		std::array<uint64_t, O + 1> oracle_candidates_by_weight{};
		std::array<uint64_t, O + 1> oracle_primitive_ops_by_weight{};
		std::array<uint64_t, O + 1> operations_by_weight{};
		std::array<uint64_t, O + 1> group_solves_by_weight{};
		std::array<uint64_t, 4> pair_parallel_cycles{};
	};
...
private:
	...
	mutable DualDecisionStats baseline_dual_stats_{};
	...
	static bool selected_mask_lexicographically_less(
		uint64_t left, uint64_t right, int weight
	)
	{
		if (!right)
			return true;
		for (int index = 0; index < weight; ++index) {
			const int left_rank = __builtin_ctzll(left);
			const int right_rank = __builtin_ctzll(right);
	...
	std::pair<int, uint64_t> solve_dual_group_weight1(
		int group,
		int suffix,
		State initial_state,
		const std::vector<int> &lambda,
	...
	) const
	{
		...
		for (int rank = suffix; rank < K; ++rank) {
			const int value =
				...
				GROUPS * this->state_cost(
					group,
					static_cast<State>(initial_state ^ this->row_mask(group, rank))
				);
			++operations;
			const uint64_t mask = uint64_t{1} << rank;
			if (value < best ||
				(value == best && selected_mask_lexicographically_less(mask, witness, 1))) {
				best = value;
				witness = mask;
			}
		}
	...
	std::pair<int, uint64_t> solve_dual_group_weight2(
		int group,
		int suffix,
		State initial_state,
		const std::vector<int> &lambda,
	...
	{
		const int remaining = K - suffix;
		const uint64_t direct_work = binomial(remaining, 2);
		const uint64_t indexed_work = remaining < 2 ? 0 :
			static_cast<uint64_t>(remaining - 1) * (STATES + 1ULL);
		int best = INFINITY_COST;
		uint64_t witness = 0;
		if (direct_work <= indexed_work) {
			for (int first = suffix; first + 1 < K; ++first)
				for (int second = first + 1; second < K; ++second) {
					const State state = static_cast<State>(
						initial_state ^ this->row_mask(group, first) ^
						this->row_mask(group, second)
					);
					const int value =
						lambda[static_cast<std::size_t>(group * K + first)] +
						lambda[static_cast<std::size_t>(group * K + second)] +
						GROUPS * this->state_cost(group, state);
					++operations;
					const uint64_t mask =
						(uint64_t{1} << first) | (uint64_t{1} << second);
					if (value < best ||
						(value == best && selected_mask_lexicographically_less(mask, witness, 2))) {
						best = value;
						witness = mask;
					}
				}
		} else {
			std::array<int, STATES> best_first_cost{};
			std::array<uint64_t, STATES> best_first_mask{};
			best_first_cost.fill(INFINITY_COST);
			for (int second = suffix + 1; second < K; ++second) {
				const int first = second - 1;
				const State first_effect = this->row_mask(group, first);
				const int first_cost =
					lambda[static_cast<std::size_t>(group * K + first)];
				const uint64_t first_mask = uint64_t{1} << first;
				++operations;
				if (first_cost < best_first_cost[static_cast<std::size_t>(first_effect)] ||
					(first_cost == best_first_cost[static_cast<std::size_t>(first_effect)] &&
					 selected_mask_lexicographically_less(
						 first_mask,
						 best_first_mask[static_cast<std::size_t>(first_effect)], 1
					 ))) {
					best_first_cost[static_cast<std::size_t>(first_effect)] = first_cost;
					best_first_mask[static_cast<std::size_t>(first_effect)] = first_mask;
				}
				const State second_effect = this->row_mask(group, second);
				const int second_cost =
					lambda[static_cast<std::size_t>(group * K + second)];
				for (int effect = 0; effect < STATES; ++effect) {
					++operations;
					if (best_first_cost[static_cast<std::size_t>(effect)] >= INFINITY_COST)
						continue;
					const int value =
						best_first_cost[static_cast<std::size_t>(effect)] + second_cost +
						GROUPS * this->state_cost(
							group, static_cast<State>(initial_state ^ effect ^ second_effect)
						);
					const uint64_t mask =
						best_first_mask[static_cast<std::size_t>(effect)] |
						(uint64_t{1} << second);
					if (value < best ||
						(value == best && selected_mask_lexicographically_less(mask, witness, 2))) {
						best = value;
	...
	std::pair<int, uint64_t> solve_dual_group_weight3(
		int group,
		int suffix,
		State initial_state,
		const std::vector<int> &lambda,
	...
	{
		const int remaining = K - suffix;
		const uint64_t direct_work = binomial(remaining, 3);
		const uint64_t indexed_work = binomial(remaining, 2) +
			static_cast<uint64_t>(std::max(0, remaining - 2)) * STATES;
		int best = INFINITY_COST;
		uint64_t witness = 0;
		if (direct_work <= indexed_work) {
			for (int first = suffix; first + 2 < K; ++first)
				for (int second = first + 1; second + 1 < K; ++second)
					for (int third = second + 1; third < K; ++third) {
						const State state = static_cast<State>(
							initial_state ^ this->row_mask(group, first) ^
							this->row_mask(group, second) ^ this->row_mask(group, third)
						);
						const int value =
							lambda[static_cast<std::size_t>(group * K + first)] +
							lambda[static_cast<std::size_t>(group * K + second)] +
							lambda[static_cast<std::size_t>(group * K + third)] +
							GROUPS * this->state_cost(group, state);
						++operations;
						const uint64_t mask = (uint64_t{1} << first) |
							(uint64_t{1} << second) | (uint64_t{1} << third);
						if (value < best ||
							(value == best && selected_mask_lexicographically_less(mask, witness, 3))) {
							best = value;
							witness = mask;
						}
					}
		} else {
			std::array<int, STATES> best_pair_cost{};
			std::array<uint64_t, STATES> best_pair_mask{};
			best_pair_cost.fill(INFINITY_COST);
			for (int third = suffix + 2; third < K; ++third) {
				const int second = third - 1;
				for (int first = suffix; first < second; ++first) {
					const State effect = static_cast<State>(
						this->row_mask(group, first) ^ this->row_mask(group, second)
					);
					const int value =
						lambda[static_cast<std::size_t>(group * K + first)] +
						lambda[static_cast<std::size_t>(group * K + second)];
					const uint64_t mask =
						(uint64_t{1} << first) | (uint64_t{1} << second);
					++operations;
					if (value < best_pair_cost[static_cast<std::size_t>(effect)] ||
						(value == best_pair_cost[static_cast<std::size_t>(effect)] &&
						 selected_mask_lexicographically_less(
							 mask, best_pair_mask[static_cast<std::size_t>(effect)], 2
						 ))) {
						best_pair_cost[static_cast<std::size_t>(effect)] = value;
						best_pair_mask[static_cast<std::size_t>(effect)] = mask;
					}
				}
				const State third_effect = this->row_mask(group, third);
				const int third_cost =
					lambda[static_cast<std::size_t>(group * K + third)];
				for (int effect = 0; effect < STATES; ++effect) {
					++operations;
					if (best_pair_cost[static_cast<std::size_t>(effect)] >= INFINITY_COST)
						continue;
					const int value =
						best_pair_cost[static_cast<std::size_t>(effect)] + third_cost +
						GROUPS * this->state_cost(
							group, static_cast<State>(initial_state ^ effect ^ third_effect)
						);
					const uint64_t mask =
						best_pair_mask[static_cast<std::size_t>(effect)] |
						(uint64_t{1} << third);
					if (value < best ||
						(value == best && selected_mask_lexicographically_less(mask, witness, 3))) {
						best = value;
	...
	std::pair<int, uint64_t> solve_dual_group_specialized(
		int group,
		int suffix,
		int target_weight,
		State initial_state,
	...
	{
		if (target_weight == 1)
			return solve_dual_group_weight1(
				group, suffix, initial_state, lambda, operations
			);
		if (target_weight == 2)
			return solve_dual_group_weight2(
				group, suffix, initial_state, lambda, operations
			);
		if (target_weight == 3)
			return solve_dual_group_weight3(
				group, suffix, initial_state, lambda, operations
			);
		return solve_dual_group(
			group, suffix, target_weight, initial_state, lambda, operations
		);
	}
...
public:
	...
	) const
	{
		...
		for (int target = 1; target <= budget && suffix + target <= K;
			...
			for (int update = 0; update < DUAL_UPDATES; ++update) {
				++baseline_dual_stats_.updates;
				if (target == 2) {
					const uint64_t pairs = binomial(K - suffix, 2);
					static constexpr std::array<int, 4> LANES{1, 4, 8, 13};
					for (std::size_t lane = 0; lane < LANES.size(); ++lane)
						baseline_dual_stats_.pair_parallel_cycles[lane] +=
							pairs * ((GROUPS + LANES[lane] - 1) / LANES[lane]);
				}
				std::array<uint64_t, GROUPS> selections{};
				int dual = GROUPS * current_information_cost;
				int primal = INFINITY_COST;
				for (int group = 0; group < GROUPS; ++group) {
					const uint64_t before = dp_transitions;
					const auto solution = solve_dual_group(
						group, suffix, target,
						this->packed_group_state(states, group),
						lambda, dp_transitions
					);
					const uint64_t delta = dp_transitions - before;
					baseline_dual_stats_.operations += delta;
					baseline_dual_stats_.operations_by_weight[static_cast<std::size_t>(target)] += delta;
					++baseline_dual_stats_.group_solves;
					++baseline_dual_stats_.group_solves_by_weight[static_cast<std::size_t>(target)];
					dual += solution.first;
	...
	{
		if (target <= 0 || suffix + target > K)
			return INFINITY_COST;
		int value = current_information_cost +
			this->minimum_information_cost(suffix, target);
		for (int group = 0; group < GROUPS; ++group) {
			++table_lookups;
			value += this->minimum(
				group, suffix, target,
	...
	) const
	{
		if (target <= 0 || suffix + target > K)
			return INFINITY_COST;
		if (suffix >= SPLIT)
			return normal_weight_bound(
				suffix, target, current_information_cost, states,
				table_lookups
			);
		int best = INFINITY_COST;
		for (int left = 0; left <= target; ++left) {
			...
			if (left_information >= INFINITY_COST ||
				right_information >= INFINITY_COST)
				continue;
			int value = current_information_cost +
				left_information + right_information;
			...
			for (int group = 0; group < GROUPS; ++group) {
				...
				int group_minimum = INFINITY_COST;
				const State current = this->packed_group_state(states, group);
				while (effects) {
					const int effect = __builtin_ctzll(effects);
					effects &= effects - 1;
					++table_lookups;
	...
	std::pair<int, uint64_t> exact_low_cardinality_oracle(
		int suffix,
		int target,
		int current_information_cost,
		const ExactStates &current_exact,
		DualDecisionStats &stats
	) const
	{
		int best = INFINITY_COST;
		uint64_t witness = 0;
		const uint64_t per_candidate_primitives =
			static_cast<uint64_t>(target) +
			static_cast<uint64_t>(target * EXACT_WORDS) +
			static_cast<uint64_t>(N - K) + 1ULL;
		auto account = [&]() {
			++stats.oracle_candidates;
			stats.oracle_metric_terms += N - K;
			stats.oracle_primitive_ops += per_candidate_primitives;
			++stats.oracle_candidates_by_weight[static_cast<std::size_t>(target)];
			stats.oracle_primitive_ops_by_weight[static_cast<std::size_t>(target)] +=
				per_candidate_primitives;
		};
		if (target == 1) {
			for (int first = suffix; first < K; ++first) {
				const uint64_t mask = uint64_t{1} << first;
				const int value = exact_completion_cost(
					mask, current_information_cost, current_exact
				);
				account();
				if (value < best ||
					(value == best && selected_mask_lexicographically_less(mask, witness, 1))) {
					best = value; witness = mask;
				}
			}
		} else if (target == 2) {
			for (int first = suffix; first + 1 < K; ++first)
	...
	static uint64_t sparse_group_operation_estimate(int remaining, int target)
	{
		if (target == 1)
			return 2ULL * static_cast<uint64_t>(remaining);
		if (target == 2)
			return static_cast<uint64_t>(std::max(0, remaining - 1)) +
				2ULL * binomial(remaining, 2);
		return binomial(remaining, target) *
			static_cast<uint64_t>(target + 1);
	}

	int fused_threshold_dual_lower_bound(
		int suffix,
		int budget,
		int current_information_cost,
		const PackedStates &states,
	...
	) const
	{
		...
		for (int target = 1; target <= budget && suffix + target <= K;
			++target) {
			const int compact = compact_weight_bound(
				suffix, target, current_information_cost, states,
				table_lookups
			);
			if (compact > incumbent) {
				++stats.compact_certificates;
				overall = std::min(overall, compact);
				continue;
			}

			bool use_exact = use_oracles && target == 1;
			if (use_oracles && target == 2) {
				const int remaining = K - suffix;
				const uint64_t oracle_cost =
					binomial(remaining, 2) * static_cast<uint64_t>(N - K);
				const uint64_t predicted_dual =
					sparse_group_operation_estimate(remaining, 2) *
					static_cast<uint64_t>(GROUPS * DUAL_UPDATES);
				use_exact = oracle_cost < predicted_dual;
			}
			if (use_exact) {
				const auto exact = exact_low_cardinality_oracle(
					suffix, target, current_information_cost,
					current_exact, stats
				);
				overall = std::min(overall, exact.first);
				if (exact.first <= incumbent) {
					++stats.witness_stops;
					return overall;
				}
				++stats.dual_certificates;
				continue;
			}

			std::vector<int> lambda(static_cast<std::size_t>(GROUPS * K));
			for (int group = 0; group < GROUPS; ++group)
				for (int rank = suffix; rank < K; ++rank)
					lambda[static_cast<std::size_t>(group * K + rank)] =
						this->information_cost(rank);
			std::array<uint64_t, GROUPS> selections{};
			std::array<int, GROUPS> group_values{};
			std::vector<int> active;
			int best_target = -INFINITY_COST;
			bool certified = false;
			for (int update = 0; update < DUAL_UPDATES; ++update) {
				++stats.updates;
				if (update == 0 || partial_groups <= 0) {
					active.resize(GROUPS);
				...
				if (target == 2) {
					...
					static constexpr std::array<int, 4> LANES{1, 4, 8, 13};
					for (std::size_t lane = 0; lane < LANES.size(); ++lane)
						stats.pair_parallel_cycles[lane] += pairs *
							((static_cast<int>(active.size()) + LANES[lane] - 1) /
							 LANES[lane]);
				}
				for (const int group : active) {
					const uint64_t before = stats.operations;
					const auto solution = solve_dual_group(
						group, suffix, target,
						this->packed_group_state(states, group),
						lambda, stats.operations
					);
					const uint64_t delta = stats.operations - before;
					stats.operations_by_weight[static_cast<std::size_t>(target)] += delta;
					++stats.group_solves;
					++stats.group_solves_by_weight[static_cast<std::size_t>(target)];
					group_values[static_cast<std::size_t>(group)] = solution.first;
					selections[static_cast<std::size_t>(group)] = solution.second;
				}
				int dual = GROUPS * current_information_cost;
				int primal = INFINITY_COST;
				for (int group = 0; group < GROUPS; ++group) {
					dual += group_values[static_cast<std::size_t>(group)];
					primal = std::min(
						primal,
						exact_completion_cost(
							selections[static_cast<std::size_t>(group)],
							current_information_cost, current_exact
						)
					);
				}
				best_target = std::max(best_target, ceil_divide(dual, GROUPS));
				if (best_target > incumbent) {
					++stats.dual_certificates;
					certified = true;
					break;
				}
				if (primal <= incumbent) {
					++stats.witness_stops;
					overall = std::min(overall, std::max(compact, best_target));
					return overall;
				}

				std::array<int, K> global_counts{};
				for (int group = 0; group < GROUPS; ++group) {
					uint64_t selected = selections[static_cast<std::size_t>(group)];
					while (selected) {
						const int rank = __builtin_ctzll(selected);
						selected &= selected - 1;
						++global_counts[static_cast<std::size_t>(rank)];
					}
				}
				if (update == 0 && partial_groups > 0 && partial_groups < GROUPS) {
					std::array<std::pair<long long, int>, GROUPS> conflict{};
					for (int group = 0; group < GROUPS; ++group) {
						long long score = 0;
						for (int rank = suffix; rank < K; ++rank) {
							const int chosen =
								(selections[static_cast<std::size_t>(group)] >> rank) & 1ULL;
							const int direction = GROUPS * chosen -
								global_counts[static_cast<std::size_t>(rank)];
							score += static_cast<long long>(direction) * direction;
						}
						conflict[static_cast<std::size_t>(group)] = {score, group};
					}
					...
					active.clear();
					for (int index = 0; index < partial_groups; ++index)
						active.push_back(conflict[static_cast<std::size_t>(index)].second);
				}

				std::array<int, K> active_counts{};
				for (const int group : active) {
					uint64_t selected = selections[static_cast<std::size_t>(group)];
					while (selected) {
						const int rank = __builtin_ctzll(selected);
						selected &= selected - 1;
						++active_counts[static_cast<std::size_t>(rank)];
					}
				}
				long long norm = 0;
				const int active_size = static_cast<int>(active.size());
				for (const int group : active)
					for (int rank = suffix; rank < K; ++rank) {
						const int chosen =
							(selections[static_cast<std::size_t>(group)] >> rank) & 1ULL;
						const int direction = active_size * chosen -
							active_counts[static_cast<std::size_t>(rank)];
						norm += static_cast<long long>(direction) * direction;
					}
				if (!norm) break;
				...
				const int step = static_cast<int>(std::min<long long>(
					DUAL_MAX_STEP,
					std::max<long long>(1, 3 * gap * active_size / (2 * norm))
				));
				for (const int group : active)
					for (int rank = suffix; rank < K; ++rank) {
						const int chosen =
							(selections[static_cast<std::size_t>(group)] >> rank) & 1ULL;
						lambda[static_cast<std::size_t>(group * K + rank)] +=
							step * (active_size * chosen -
								active_counts[static_cast<std::size_t>(rank)]);
					}
			}
			const int per_weight = std::max(compact, best_target);
			overall = std::min(overall, per_weight);
			if (!certified)
				return overall;
		}
		return overall;
	...
	void account_dual_witness(
		int target, DualDecisionStats &stats
	) const
	{
		++stats.witness_candidates;
		stats.witness_metric_terms += N - K;
		stats.witness_primitive_ops +=
			static_cast<uint64_t>(target) +
			static_cast<uint64_t>(target * EXACT_WORDS) +
			static_cast<uint64_t>(N - K) + 1ULL;
	}

	int r3_specialized_bound(
		int suffix,
		int budget,
		int current_information_cost,
		const PackedStates &states,
		...
		int incumbent,
		bool fused,
		int oracle_max_weight,
		uint64_t &table_lookups,
		DualDecisionStats &stats
	) const
	{
		int compact_overall = INFINITY_COST;
		if (!fused) {
			for (int target = 1; target <= budget && suffix + target <= K; ++target)
				compact_overall = std::min(
					compact_overall,
					compact_weight_bound(
						suffix, target, current_information_cost, states, table_lookups
					)
				);
			if (compact_overall > incumbent) {
				++stats.compact_certificates;
				return compact_overall;
			}
		}

		int overall = INFINITY_COST;
		for (int target = 1; target <= budget && suffix + target <= K; ++target) {
			int compact = compact_overall;
			if (fused) {
				compact = compact_weight_bound(
					suffix, target, current_information_cost, states, table_lookups
				);
				if (compact > incumbent) {
					++stats.compact_certificates;
			...
			if (fused && target <= oracle_max_weight) {
				const auto exact = exact_low_cardinality_oracle(
					suffix, target, current_information_cost, current_exact, stats
				);
				const int per_weight = std::max(compact, exact.first);
				overall = std::min(overall, per_weight);
				if (exact.first > incumbent) {
					++stats.dual_certificates;
					continue;
			...
			for (int update = 0; update < DUAL_UPDATES; ++update) {
				...
				for (int group = 0; group < GROUPS; ++group) {
					const uint64_t before = stats.operations;
					const auto solution = solve_dual_group_specialized(
						group, suffix, target,
						this->packed_group_state(states, group), lambda, stats.operations
					);
					const uint64_t delta = stats.operations - before;
					stats.operations_by_weight[static_cast<std::size_t>(target)] += delta;
					++stats.group_solves;
					...
					selections[static_cast<std::size_t>(group)] = solution.second;
					dual += solution.first;
					account_dual_witness(target, stats);
					primal = std::min(
						primal,
						exact_completion_cost(
							solution.second, current_information_cost, current_exact
						)
					);
				}
				best_target = std::max(best_target, ceil_divide(dual, GROUPS));
				...
				for (int group = 0; group < GROUPS; ++group)
					for (int rank = suffix; rank < K; ++rank) {
						const int chosen =
							(selections[static_cast<std::size_t>(group)] >> rank) & 1ULL;
						const int direction = GROUPS * chosen - counts[static_cast<std::size_t>(rank)];
						norm += static_cast<long long>(direction) * direction;
					}
				if (!norm) break;
				const long long gap = std::max<long long>(
				...
				const int step = static_cast<int>(std::min<long long>(
					DUAL_MAX_STEP,
					std::max<long long>(1, 3 * gap * GROUPS / (2 * norm))
				));
				for (int group = 0; group < GROUPS; ++group)
					for (int rank = suffix; rank < K; ++rank) {
						const int chosen =
							(selections[static_cast<std::size_t>(group)] >> rank) & 1ULL;
						lambda[static_cast<std::size_t>(group * K + rank)] +=
							step * (GROUPS * chosen - counts[static_cast<std::size_t>(rank)]);
					}
			}
			const int per_weight = std::max(compact, best_target);
			overall = std::min(overall, per_weight);
		...
		return fused ? overall : std::max(compact_overall, overall);
	}

	uint64_t r3_structural_solver_audit(uint64_t seed, int trials) const
	{
		std::mt19937_64 random(seed);
		uint64_t passed = 0;
		for (int trial = 0; trial < trials; ++trial) {
			const int target = 1 + static_cast<int>(random() % 3ULL);
			const int remaining = target + static_cast<int>(random() % 10ULL);
			const int suffix = K - remaining;
			const int group = static_cast<int>(random() % GROUPS);
			const State initial_state = static_cast<State>(random() % STATES);
			std::vector<int> lambda(static_cast<std::size_t>(GROUPS * K));
			for (int g = 0; g < GROUPS; ++g)
				for (int rank = suffix; rank < K; ++rank)
					lambda[static_cast<std::size_t>(g * K + rank)] =
						this->information_cost(rank);
			for (int rank = suffix; rank < K; ++rank) {
				for (int perturb = 0; perturb < 3; ++perturb) {
					const int first = static_cast<int>(random() % GROUPS);
					int second = static_cast<int>(random() % GROUPS);
					if (second == first) second = (second + 1) % GROUPS;
					const int delta = static_cast<int>(random() % 65ULL) - 32;
					lambda[static_cast<std::size_t>(first * K + rank)] += delta;
					lambda[static_cast<std::size_t>(second * K + rank)] -= delta;
				}
			}
			uint64_t specialized_ops = 0;
			uint64_t brute_ops = 0;
			const auto specialized = solve_dual_group_specialized(
				group, suffix, target, initial_state, lambda, specialized_ops
			);
			const auto brute = solve_dual_group_sparse(
				group, suffix, target, initial_state, lambda, brute_ops
			);
			assert(specialized.first == brute.first);
			const auto objective = [&](uint64_t selected) {
				State state = initial_state;
				int value = 0;
				while (selected) {
					const int rank = __builtin_ctzll(selected);
					selected &= selected - 1;
					state = static_cast<State>(state ^ this->row_mask(group, rank));
					value += lambda[static_cast<std::size_t>(group * K + rank)];
				}
				return value + GROUPS * this->state_cost(group, state);
			};
			assert(objective(specialized.second) == specialized.first);
			++passed;
		}
		return passed;
	}

	uint64_t r3_structural_w3_solver_audit(uint64_t seed, int trials) const
	{
		std::mt19937_64 random(seed);
		uint64_t passed = 0;
		for (int trial = 0; trial < trials; ++trial) {
			constexpr int target = 3;
			const int remaining = 25 + static_cast<int>(random() % 40ULL);
			const int suffix = K - remaining;
			const int group = static_cast<int>(random() % GROUPS);
			const State initial_state = static_cast<State>(random() % STATES);
			std::vector<int> lambda(static_cast<std::size_t>(GROUPS * K));
	...
	uint64_t r3_structural_bound_audit(uint64_t seed, int trials) const
	{
		std::mt19937_64 random(seed);
		uint64_t passed = 0;
		for (int trial = 0; trial < trials; ++trial) {
			const int remaining = 5 + static_cast<int>(random() % 8ULL);
			const int suffix = K - remaining;
			const int budget = 1 + static_cast<int>(random() %
				static_cast<uint64_t>(std::min(3, remaining)));
			PackedStates states = this->initial_packed_states();
			ExactStates exact = this->initial_exact_states();
			int information = 0;
			const int prefix_choices = static_cast<int>(random() % 4ULL);
			uint64_t prefix_mask = 0;
			for (int choice = 0; choice < prefix_choices; ++choice) {
				const int rank = static_cast<int>(random() % static_cast<uint64_t>(suffix));
				if ((prefix_mask >> rank) & 1ULL) continue;
				prefix_mask |= uint64_t{1} << rank;
				information += this->information_cost(rank);
				for (int word = 0; word < PACKED_WORDS; ++word)
					states[static_cast<std::size_t>(word)] ^=
						this->packed_row_mask(rank, word);
				for (int word = 0; word < EXACT_WORDS; ++word)
					exact[static_cast<std::size_t>(word)] ^=
						this->exact_row_mask(rank, word);
			}
			int exact_minimum = INFINITY_COST;
			for (int target = 1; target <= budget; ++target) {
				auto enumerate = [&](auto &&self, int next, int left,
					uint64_t selected) -> void {
					if (!left) {
						exact_minimum = std::min(
							exact_minimum,
							exact_completion_cost(selected, information, exact)
						);
						return;
					}
					for (int rank = next; rank + left <= K; ++rank)
						self(self, rank + 1, left - 1,
							selected | (uint64_t{1} << rank));
				};
				enumerate(enumerate, suffix, target, 0);
			}
			assert(exact_minimum < INFINITY_COST);
			for (int configuration = 0; configuration < 4; ++configuration) {
				for (int threshold_case = 0; threshold_case < 3; ++threshold_case) {
					DualDecisionStats stats{};
					uint64_t lookups = 0;
					const bool fused = configuration != 0;
					const int oracle_max = configuration == 2 ? 1 :
						configuration == 3 ? 2 : 0;
					const int incumbent = threshold_case == 0
						? exact_minimum - 1
						: threshold_case == 1
							? exact_minimum
							: exact_minimum + 1000;
					const int bound = r3_specialized_bound(
						suffix, budget, information, states, exact, incumbent,
						fused, oracle_max, lookups, stats
					);
					// Fused early-witness exits are threshold decisions, not
					// reusable numerical lower bounds.  The decoder-safety
					// invariant is that PRUNE implies the exact subtree minimum
					// is strictly above the captured incumbent.
					assert(!(bound > incumbent) || exact_minimum > incumbent);
					if (!fused)
						assert(bound <= exact_minimum);
					++passed;
				}
			}
		}
	...
	const DualDecisionStats &baseline_dual_stats() const
	{
		return baseline_dual_stats_;
	}
...
class ReconstructedPdbCertificationSearch {
	...
	typename Pdb::DualDecisionStats redesign_stats_{};
	std::vector<uint64_t> evaluated_masks_;
			...
			if constexpr (DEPTH + 1 < O) {
				...
				if (suffix < K) {
					...
					if (lower <= best_distance_) {
						...
						} else if constexpr (BOUND_KIND >= 9) {
							++dual_queries_;
							constexpr bool FUSED = BOUND_KIND != 9;
							constexpr int ORACLE_MAX =
								BOUND_KIND == 11 ? 1 : BOUND_KIND == 12 ? 2 : 0;
							lower = pdb_.r3_specialized_bound(
								suffix, budget, information_cost_, states_, exact_,
								best_distance_, FUSED, ORACLE_MAX, compact_lookups_,
								redesign_stats_
							);
							dual_transitions_ = redesign_stats_.operations;
						} else if constexpr (BOUND_KIND >= 4) {
							++dual_queries_;
							constexpr bool USE_ORACLES = BOUND_KIND == 5;
							constexpr int PARTIAL = BOUND_KIND == 6 ? 4 :
								BOUND_KIND == 7 ? 6 : BOUND_KIND == 8 ? 8 : 0;
							lower = pdb_.fused_threshold_dual_lower_bound(
								suffix, budget, information_cost_, states_,
								exact_, best_distance_, USE_ORACLES, PARTIAL,
								compact_lookups_, redesign_stats_
							);
							dual_transitions_ = redesign_stats_.operations;
						} else {
...
public:
	...
	const typename Pdb::DualDecisionStats &redesign_stats() const
	{
		return redesign_stats_;
	}
	const std::vector<uint64_t> &evaluated_masks() const
	{
		return evaluated_masks_;
...
)
{
...
#ifdef OSD_R3_DECISIVE_AUDIT
	static bool r3_audited = false;
	if (!r3_audited) {
		override_stats.r3_structural_checks =
			pdb.r3_structural_solver_audit(0x52335f5350454349ULL, 512) +
			pdb.r3_structural_w3_solver_audit(0x52335f57335f4c47ULL, 512) +
			pdb.r3_structural_bound_audit(0x52335f424f554e44ULL, 256);
		r3_audited = true;
	}
#endif
	...
	const auto &baseline_dual_stats = pdb.baseline_dual_stats();
	const auto &redesign_stats = certify.redesign_stats();
	const bool redesigned = BOUND_KIND >= 4;
	const auto &chosen_stats = redesigned ? redesign_stats : baseline_dual_stats;
	override_stats.dual_updates_executed = chosen_stats.updates;
	override_stats.dual_group_solves = chosen_stats.group_solves;
	override_stats.compact_weight_certificates = chosen_stats.compact_certificates;
	override_stats.dual_weight_certificates = chosen_stats.dual_certificates;
	override_stats.witness_early_stops = chosen_stats.witness_stops;
	override_stats.exact_oracle_candidates = chosen_stats.oracle_candidates;
	override_stats.exact_oracle_metric_terms = chosen_stats.oracle_metric_terms;
	override_stats.exact_oracle_primitive_ops = chosen_stats.oracle_primitive_ops;
	override_stats.dual_witness_candidates = chosen_stats.witness_candidates;
	override_stats.dual_witness_metric_terms = chosen_stats.witness_metric_terms;
	override_stats.dual_witness_primitive_ops = chosen_stats.witness_primitive_ops;
	for (int weight = 0; weight <= O && weight < 5; ++weight) {
		override_stats.exact_oracle_candidates_by_weight[static_cast<std::size_t>(weight)] =
			chosen_stats.oracle_candidates_by_weight[static_cast<std::size_t>(weight)];
		override_stats.exact_oracle_primitive_ops_by_weight[static_cast<std::size_t>(weight)] =
			chosen_stats.oracle_primitive_ops_by_weight[static_cast<std::size_t>(weight)];
		override_stats.dual_operations_by_weight[static_cast<std::size_t>(weight)] =
			chosen_stats.operations_by_weight[static_cast<std::size_t>(weight)];
		override_stats.dual_group_solves_by_weight[static_cast<std::size_t>(weight)] =
			chosen_stats.group_solves_by_weight[static_cast<std::size_t>(weight)];
	}
	override_stats.pair_parallel_cycles = chosen_stats.pair_parallel_cycles;
...
bool is_exact_mode(Mode mode)
{
	return mode == Mode::baseline || mode == Mode::exact_stop_only ||
		...
		mode == Mode::reconstructed_fused_threshold_dual_exact ||
		mode == Mode::reconstructed_fused_oracle_dual_exact ||
		mode == Mode::reconstructed_partial_h4_dual_exact ||
		mode == Mode::reconstructed_partial_h6_dual_exact ||
		mode == Mode::reconstructed_partial_h8_dual_exact ||
		mode == Mode::r3_specialized_dual_exact ||
		mode == Mode::r3_fused_specialized_dual_exact ||
		mode == Mode::r3_fused_w1_oracle_exact ||
		mode == Mode::r3_fused_w1w2_oracle_exact ||
...
)
{
	...
	if (
		...
		active_mode == Mode::reconstructed_compact_two_block_pdb_exact ||
		active_mode == Mode::reconstructed_dual_two_block_pdb_exact ||
		active_mode == Mode::reconstructed_fused_threshold_dual_exact ||
		active_mode == Mode::reconstructed_fused_oracle_dual_exact ||
		active_mode == Mode::reconstructed_partial_h4_dual_exact ||
		active_mode == Mode::reconstructed_partial_h6_dual_exact ||
		active_mode == Mode::reconstructed_partial_h8_dual_exact ||
		active_mode == Mode::r3_specialized_dual_exact ||
		active_mode == Mode::r3_fused_specialized_dual_exact ||
		active_mode == Mode::r3_fused_w1_oracle_exact ||
		active_mode == Mode::r3_fused_w1w2_oracle_exact ||
		...
		else if (active_mode == Mode::reconstructed_fused_threshold_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 4>(
				matrix, base, candidate, soft, best, next,
				active_guard_candidate_budget
			);
		else if (active_mode == Mode::reconstructed_fused_oracle_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 5>(
				matrix, base, candidate, soft, best, next,
				active_guard_candidate_budget
			);
		else if (active_mode == Mode::reconstructed_partial_h4_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 6>(
				matrix, base, candidate, soft, best, next,
				active_guard_candidate_budget
			);
		else if (active_mode == Mode::reconstructed_partial_h6_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 7>(
				matrix, base, candidate, soft, best, next,
				active_guard_candidate_budget
			);
		else if (active_mode == Mode::reconstructed_partial_h8_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 8>(
				matrix, base, candidate, soft, best, next,
				active_guard_candidate_budget
			);
		else if (active_mode == Mode::r3_specialized_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 9>(
				matrix, base, candidate, soft, best, next, active_guard_candidate_budget
			);
		else if (active_mode == Mode::r3_fused_specialized_dual_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 10>(
				matrix, base, candidate, soft, best, next, active_guard_candidate_budget
			);
		else if (active_mode == Mode::r3_fused_w1_oracle_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 11>(
				matrix, base, candidate, soft, best, next, active_guard_candidate_budget
			);
		else if (active_mode == Mode::r3_fused_w1w2_oracle_exact)
			completed = execute_reconstructed_pdb_search<N, K, O, 12>(
				matrix, base, candidate, soft, best, next, active_guard_candidate_budget
			);
		else
