...
struct OverrideStats {
	...
	uint64_t incumbent_syncs = 0;
	uint64_t incumbent_batch_items = 0;
...
#endif
#ifndef OSD_RECONSTRUCTED_INCUMBENT_BATCH
#define OSD_RECONSTRUCTED_INCUMBENT_BATCH 1
#endif
template <int N, int K, int O>
...
class ReconstructedPdbCertificationSearch {
	...
	int best_distance_;
	int next_distance_;
	int pending_best_distance_;
	int pending_next_distance_;
	uint64_t incumbent_batch_items_ = 0;
	uint64_t incumbent_syncs_ = 0;
	...
	void synchronize_incumbent()
	{
		if (!incumbent_batch_items_)
			return;
		best_distance_ = pending_best_distance_;
		next_distance_ = pending_next_distance_;
		incumbent_batch_items_ = 0;
		++incumbent_syncs_;
	}
	...
	int evaluate(int weight)
	{
		...
		if (!is_seed(weight)) {
			...
			if (distance < pending_best_distance_) {
				pending_next_distance_ = pending_best_distance_;
				pending_best_distance_ = distance;
				copy_candidate(weight);
			} else if (distance < pending_next_distance_) {
				pending_next_distance_ = distance;
			}
		}
		++incumbent_batch_items_;
		if (
			incumbent_batch_items_ >=
			static_cast<uint64_t>(OSD_RECONSTRUCTED_INCUMBENT_BATCH)
		)
			synchronize_incumbent();
		return parity;
	}
...
public:
		...
		exact_(pdb.initial_exact_states())
	{
		...
		pending_best_distance_ = best_distance_;
		pending_next_distance_ = next_distance_;
		for (int rank = 0; rank < K; ++rank) {
			const int row = pdb.coordinate(rank);
			if (base[row] != output[row])
	...
	uint64_t incumbent_syncs() const { return incumbent_syncs_; }
	uint64_t incumbent_batch_items() const
	{
		return scoring_calls_;
	}
	const std::vector<uint64_t> &evaluated_masks() const
	{
		return evaluated_masks_;
...
)
{
	...
	override_stats.incumbent_syncs = certify.incumbent_syncs();
	override_stats.incumbent_batch_items = certify.incumbent_batch_items();
