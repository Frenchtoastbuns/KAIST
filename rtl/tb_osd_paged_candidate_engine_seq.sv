module tb_osd_paged_candidate_engine_seq;
	...
	osd_paged_candidate_engine_seq #(
		.K(K),
		.R(R),
		.P(P),
		.LLR_W(LLR_W),
	...
	initial begin
		...
		while (!$feof(vectors)) begin
			...
			if (fields == 12) begin
				...
				#1;
				if (busy !== 1'b0)
					$fatal(1, "busy remained asserted at case %0d", cases);
		...
		$display(
			"RTL_SEQ_PASS cases=%0d K=%0d R=%0d P=%0d cycles_per_page=%0d",
			cases,
			K,
			R,
			P,
